...
async function cleanAndSeedGrants() {
  try {
    ...
    // Delete ALL grants
    const result = await mongoose.connection.db.collection('grants').deleteMany({})
    ...
    const inserted = await mongoose.connection.db.collection('grants').insertMany(grants)
    console.log(`✅ Inserted ${inserted.insertedCount} clean grants`)
